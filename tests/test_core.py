...
def test_registration_number():
    ...
    with pytest.raises(ValueError): normalize_registration_number("123")

def test_identity_number_wins():
    r = IdentityResolver()
    assert r.matches({"company_number":"1","legal_name":"A AS"},{"company_number":"1","legal_name":"Other"})
    assert not r.matches({"company_number":"1","legal_name":"A AS"},{"company_number":"2","legal_name":"A AS"})
...
def test_freshness_preserves_history():
    facts = [{"field":"employees","value":42,"published_at":"2025-01-01"},{"field":"employees","value":61,"published_at":"2026-01-01"}]
    result = FreshnessChecker().choose_current(facts)
    assert [f for f in result if f["is_current"]][0]["value"] == 61
