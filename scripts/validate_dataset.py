...
total = valid = evidence_facts = total_facts = identity_verified = financial_facts = financial_evidence = 0
...
if path.exists():
    for line in path.read_text().splitlines():
        ...
        try:
            ...
            financial = [f for f in facts if f.get("field") in {"revenue", "profit", "assets", "liabilities", "equity", "employees"}]
            ...
            urls_valid = all(urlparse(e.get("source_url", "")).scheme in {"http", "https"} for f in facts for e in f.get("evidence", []) if isinstance(e, dict))
...
print(f"Evidence coverage: {pct(evidence_facts, total_facts)}% Identity verification: {pct(identity_verified, total)}% Financial evidence coverage: {pct(financial_evidence, financial_facts)}%")
