...
class ResearchGraph:
    ...
    async def execute(self, company_number: str) -> ResearchState:
        ...
        try:
            ...
            state["errors"].append(f"Research workflow error: {e!s}")
            state["finished_at"] = datetime.now(UTC)
    ...
    async def _resolve_company(self, state: ResearchState) -> ResearchState:
        ...
        try:
            ...
            state["errors"].append(f"Registry resolution failed: {e!s}")
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    state["errors"].append(f"Parallel research error: {result!s}")
                else:
            ...
            state["errors"].append(f"Parallel research failed: {e!s}")
    ...
    async def _verify_facts(self, state: ResearchState) -> ResearchState:
        ...
        try:
            ...
            state["errors"].append(f"Fact verification failed: {e!s}")
    ...
    async def _apply_freshness(self, state: ResearchState) -> ResearchState:
        ...
        try:
            ...
            state["errors"].append(f"Freshness check failed: {e!s}")
