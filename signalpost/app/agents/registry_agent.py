...
class RegistryAgent:
    ...
    def extract(self, payload: dict[str, Any], source_url: str) -> dict[str, Any]:
        ...
        return {
            "company_number": str(payload.get("organizationNumber") or payload.get("organisasjonsnummer") or payload.get("company_number") or ""),
            "legal_name": payload.get("name") or payload.get("navn") or payload.get("legal_name"),
            "status": payload.get("status") or ("active" if payload.get("registrertIEnhetsregisteret") else None),
            ...
            "organization_type": (payload.get("organizationForm") or payload.get("organisasjonsform", {}).get("beskrivelse") if isinstance(payload.get("organisasjonsform"), dict) else payload.get("organisasjonsform")) or payload.get("organization_type"),
            "source_url": source_url,
        }

    async def research(self, company_number: str) -> dict[str, Any]:
        result: dict[str, Any] = {"company_identity": {}, "facts": [], "source_documents": [], "errors": [], "request_count": 0, "search_count": 0}
        try:
            ...
            for item in results:
                ...
                if identity.get("company_number") != company_number:
                    result["errors"].append("Registry response company number did not match requested number")
                    continue
                result["company_identity"] = identity
                result["source_documents"].append({"url": item["url"], "content": payload, "retrieved_at": datetime.utcnow().isoformat(), "source": self.source.name})
                result["facts"] = [fact.model_dump(mode="json") for fact in self.extractor.extract_from_registry(payload, item["url"])]
