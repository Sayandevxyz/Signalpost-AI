...
class ResearchCoordinator:
    ...
    async def research(self, company_number: str) -> ResearchState:
        ...
        for source in self.sources:
            ...
            except Exception as exc:  # noqa: BLE001 - source adapters are untrusted boundaries
                state["errors"].append(f"{source.name}: {exc}")
        state.setdefault(
            "company_identity", {"company_number": company_number, "country": "Norway"}
        )
        state["verified_facts"] = [
            fact
            for fact in state["candidate_facts"]
            if self.evidence.verify(fact, fact["evidence"])
        ]
